    ...
    override def run(): Unit = SparkHadoopUtil.get.runAsUser(sparkUser) { () =>
      ...
      try {
        ...
        for (m <- task.metrics) {
          ...
          m.deserializationStart = startTime
          m.deserializationEnd = taskStart
          m.executionStart = taskStart
          m.executionEnd = taskFinish
          m.jvmGCStart = startGCTime
          m.jvmGCEnd = gcTime
          m.serializationStart = beforeSerialization
          m.serializationEnd = afterSerialization
