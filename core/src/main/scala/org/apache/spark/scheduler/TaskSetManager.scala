...
import java.io.{FileWriter, NotSerializableException}
  ...
  extends Schedulable with Logging
{
  ...
  def handleSuccessfulTask(tid: Long, result: DirectTaskResult[_]) = {
    ...
    if (!successful(index)) {
      ...
      dumpFile.write(exeStart + "\t" + exeEnd + "\tExecution of TID %s: ".format(tid)
        + exeTime + " ms on %s\n".format(info.host))
      dumpFile.write(gcStart + "\t" + gcEnd + "\tGarbage Collection of TID %s: ".format(tid)
        + gcTime + " ms on %s\n".format(info.host))
